...
class InterestMitigationStrategyPrintStuff :
    public BestRoute
{
public:
   ...
   */
  InterestMitigationStrategyPrintStuff ()
  {
    Simulator::Schedule(Seconds(1.0), &InterestMitigationStrategyPrintStuff::IfaDetect, this);
  }
...
private:
  ...
  double m_threshold;
  std::list<std::string> prefixes_seen; // prefixes seen in the last second
...
TypeId InterestMitigationStrategyPrintStuff::GetTypeId (void)
{
  static TypeId tid = TypeId ((super::GetTypeId ().GetName ()+"::InterestMitigationStrategyPrintStuff").c_str ())
    .SetGroupName ("Ndn")
    .template SetParent <super> ()
    .template AddConstructor <InterestMitigationStrategyPrintStuff> ()
    .AddAttribute ("Threshold", "Threshold of Gini impurity increase above which IFA is detected",
                            DoubleValue (0.001),
                            MakeDoubleAccessor (&InterestMitigationStrategyPrintStuff::m_threshold),
                            MakeDoubleChecker<double> ());
  return tid;
}

std::string InterestMitigationStrategyPrintStuff::GetLogName ()
{
  return super::GetLogName () + ".InterestMitigationStrategyPrintStuff";
}

bool InterestMitigationStrategyPrintStuff::CanSendOutInterest (Ptr<Face> inFace,
                                                                 Ptr<Face> outFace,
                                                                 Ptr<const Interest> interest,
                                                                 Ptr<pit::Entry> pitEntry)
{
  std::string init_prefix = interest->GetName().getPrefix(1).toUri();
    ...
    if (std::find(prefixes_seen.begin(), prefixes_seen.end(), init_prefix) == prefixes_seen.end()) {
        // add interest
        this->prefixes_seen.push_back(init_prefix);
    }
...
void InterestMitigationStrategyPrintStuff::ClearStats()
{
    this->interest_counts = std::map<std::string, uint64_t>();
    this->prefixes_seen = std::list<std::string>();
}

double InterestMitigationStrategyPrintStuff::GiniImpurity(std::map<std::string, uint64_t> interestCounts)
{
...
std::string InterestMitigationStrategyPrintStuff::IfaFindPrefix()
{
    // to do: implement malicious prefix detection algorithm
    return "/forged";
}

bool InterestMitigationStrategyPrintStuff::IfaCheck()
{
...
void InterestMitigationStrategyPrintStuff::IfaDetect()
{
    // remove a prefix from the list of bad prefixes if it hasn't been seen in the last second
  std::list<std::string> prefixes_to_remove;
  for (auto prefix : this->bad_prefixes) {
    if (std::find(prefixes_seen.begin(), prefixes_seen.end(), prefix) == prefixes_seen.end())
      prefixes_to_remove.push_back(prefix);
  }
  for (auto prefix : prefixes_to_remove) {
    this->bad_prefixes.remove(prefix);
  }
    if (IfaCheck()) {
...
void InterestMitigationStrategyPrintStuff::SendWarning(std::string badPrefix) {
...
void InterestMitigationStrategyPrintStuff::DidReceiveUnsolicitedData(Ptr< Face > inFace, Ptr< const Data > data, bool didCreateCacheEntry) {
