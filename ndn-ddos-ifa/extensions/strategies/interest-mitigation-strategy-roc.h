// records false positives (attacks detected in the first 10 seconds) and false negatives (next 10 seconds having elapsed without a detection)
...
class InterestMitigationStrategyRoc :
    public BestRoute
{
public:
   ...
   */
  InterestMitigationStrategyRoc ()
  {
    Simulator::Schedule(Seconds(1.0), &InterestMitigationStrategyRoc::IfaDetect, this);
  }
...
TypeId InterestMitigationStrategyRoc::GetTypeId (void)
{
  static TypeId tid = TypeId ((super::GetTypeId ().GetName ()+"::InterestMitigationStrategyRoc").c_str ())
    .SetGroupName ("Ndn")
    .template SetParent <super> ()
    .template AddConstructor <InterestMitigationStrategyRoc> ()
    .AddAttribute ("Threshold", "Threshold of Gini impurity increase above which IFA is detected",
                            DoubleValue (0.001),
                            MakeDoubleAccessor (&InterestMitigationStrategyRoc::m_threshold),
                            MakeDoubleChecker<double> ());
  return tid;
}

std::string InterestMitigationStrategyRoc::GetLogName ()
{
  return super::GetLogName () + ".InterestMitigationStrategyRoc";
}

bool InterestMitigationStrategyRoc::CanSendOutInterest (Ptr<Face> inFace,
                                                                 Ptr<Face> outFace,
                                                                 Ptr<const Interest> interest,
                                                                 Ptr<pit::Entry> pitEntry)
{
...
void InterestMitigationStrategyRoc::ClearStats()
{
    this->interest_counts = std::map<std::string, uint64_t>();
}

double InterestMitigationStrategyRoc::GiniImpurity(std::map<std::string, uint64_t> interestCounts)
{
...
std::string InterestMitigationStrategyRoc::IfaFindPrefix()
{
    // to do: implement malicious prefix detection algorithm
    return "/forged";
}

bool InterestMitigationStrategyRoc::IfaCheck()
{
...
void InterestMitigationStrategyRoc::IfaDetect()
{
    ++n_seconds;
    if (IfaCheck()) {
        ...
        // print info if there was a false positive
        if (2 <= n_seconds && n_seconds <= 10) {
            std::cout << "false positive" << std::endl;
        }
        if (11 <= n_seconds && n_seconds <= 20) {
            true_positive = true;
        }
    }
    else {
    ...
    if (n_seconds == 20 /* replace with 11, 12, etc. */) {
        if (true_positive) {
...
void InterestMitigationStrategyRoc::SendWarning(std::string badPrefix) {
...
void InterestMitigationStrategyRoc::DidReceiveUnsolicitedData(Ptr< Face > inFace, Ptr< const Data > data, bool didCreateCacheEntry) {
