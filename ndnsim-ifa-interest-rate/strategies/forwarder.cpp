...
// This is a modified version of the file forwarder.cpp in order to implement the expired PIT mechanism.
// I should probably try to use forwarding strategies instead, though.
  ...
  , m_csFace(face::makeNullFace(FaceUri("contentstore://")))
{
  ...
  /* for (auto face_ptr = m_faceTable.begin(); face_ptr != m_faceTable.end(); face_ptr++) {
    std::string uri = face_ptr->getLocalUri().toString();
    std::cout << uri;
    if (uri == "netdev://[00:00:00:00:00:02]" ||
      uri == "netdev://[00:00:00:00:00:04]" ||
      uri == "netdev://[00:00:00:00:00:06]" ||
      uri == "netdev://[00:00:00:00:00:08]") {
        is_correct_node = true;
      }
  }*/

    auto ToDoOnInterestTimeout = [this](const pit::Entry& entry) {
        for (auto inRecord = entry.getInRecords().begin(); inRecord != entry.getInRecords().end(); ++inRecord) {
            // find the user/sender of an interest
            std::string owner = inRecord->getFace().getRemoteUri().getHost();
            std::cout << "Timeout from owner: " << owner << std::endl;
            // uptick count of interests from that user
    ...
    beforeExpirePendingInterest.connect(ToDoOnInterestTimeout);
    ns3::Simulator::Schedule(ns3::Seconds(1.0), &Forwarder::CheckInterestRate, this);
}

// every second
void Forwarder::CheckInterestRate() {
    for (std::pair<std::string, uint64_t> entry : this->expiry_table) {
        std::cout << "Checking the expired-interest rate of " << entry.first << " ... it's " << entry.second << std::endl;
        if ((entry.second >= 40) &&
          std::find(this->badUsers.begin(), this->badUsers.end(), entry.first) == this->badUsers.end()) {
            this->badUsers.push_back(entry.first);
        }
    }
    // clear expiry table
...
{
    std::string owner = ingress.face.getRemoteUri().getHost();
    if (std::find(this->badUsers.begin(), this->badUsers.end(), owner) != this->badUsers.end()) {
        return;
    }
