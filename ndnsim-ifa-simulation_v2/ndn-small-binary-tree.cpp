...
int main(int argc, char *argv[])
{
    ...
    // let's try printing the size of the FIB in each case.
    nfd::fib::Fib& the_fib = Names::Find<Node>("NodeR1")->GetObject<ndn::L3Protocol>()->getForwarder()->getFib();
    std::cout << "FIB size before choices: " << the_fib.size() << std::endl;
    auto printEntry = [](nfd::fib::Entry const &entry) { std::cout << entry.getPrefix() << std::endl; };
    std::for_each(the_fib.begin(), the_fib.end(), printEntry);
    ...
    std::cout << "FIB size after choices: " << Names::Find<Node>("NodeR1")->GetObject<ndn::L3Protocol>()->getForwarder()->getFib().size() << std::endl;
    std::for_each(the_fib.begin(), the_fib.end(), printEntry);
