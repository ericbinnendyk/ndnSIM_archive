...
main(int argc, char* argv[])
{
  ...
  consumerHelper.SetAttribute("Frequency", StringValue("10")); // 10 interests a second
  apps = consumerHelper.Install(nodes.Get(3));
  apps.Stop(Seconds(10.0)); // stop the consumer app at 10 seconds mark
  ...
  // App that counts the number of interests reaching the producer
  ndn::AppHelper interestCounterHelper("ns3::ndn::InterestCounter");
  interestCounterHelper.SetPrefix("/prefix");
  interestCounterHelper.SetAttribute("Period", DoubleValue(0.1));
  interestCounterHelper.Install(nodes.Get(1));
